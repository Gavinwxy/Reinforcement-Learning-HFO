...
if __name__ == '__main__':
	...
	itlen = (len(lr_max)*len(lr_min))
	#itlen = (len(lr)*len(k))
	t = tqdm(total=itlen)
	for arg in itertools.product(lr_max, lr_min):

		agents = []
		for i in range(args.numAgents):
			agent = WolfPHCAgent(learningRate = 0.1, discountFactor = 0.99, winDelta=0.01, loseDelta=0.1)
			agents.append(agent)
		...
		for episode in range(numEpisodes):	
			...
			while status[0]=="IN_GAME":
				for agent in agents:
					#loseDelta, winDelta, learningRate = agent.computeHyperparameters_Exp(arg, episode)
					loseDelta, winDelta, learningRate = agent.computeHyperparameters_cosAn(arg, episode, numEpisodes)
	...
	np.save('PHC_best_cosAn.npy', best_setting)
	#np.save('PHC_best_Exp.npy', best_setting)